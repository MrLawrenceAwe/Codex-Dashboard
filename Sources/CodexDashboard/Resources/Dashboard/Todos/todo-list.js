const todoList = (() => {
  ...
  function render() {
    ...
    if (!visible.length) {
      const message = filterMode === 'completed'
        ? 'No completed to-dos'
        : filterMode === 'open' ? 'Nothing left to do' : 'No to-dos yet';
      list.innerHTML = `<div class="todo-empty"><strong>${message}</strong><span>Add a to-do above to keep work moving.</span></div>`;
      return;
    }
      ...
      <div class="todo-shell">
        ...
        <div class="todo-toolbar">
          <div class="todo-filters" aria-label="Filter to-dos">
            <button type="button" data-todo-filter="open" class="is-active">Open</button>
            <button type="button" data-todo-filter="all">All</button>
            <button type="button" data-todo-filter="completed">Completed</button>
          </div>

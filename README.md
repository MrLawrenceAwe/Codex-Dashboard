...
All controls are available from the menu bar, with separate Diagnostics and Completion Inbox windows available on demand. Launch at Login is optional. **Completion Inbox** keeps the latest observed completion for each task until explicitly dismissed, with **Open task**, individual dismiss, and **Dismiss all** actions. Opening a task does not dismiss its completion. The menu-bar count shows undismissed tasks, and the inbox persists across dashboard relaunches in local app preferences.

Choose **On Task Completion → Silent badge**, **Notification**, or **Bring Codex to front** from the menu bar or the inbox. Foreground is the default; it respects typing activity and keeps an already-open dashboard or To-dos page in place. Explicit **Open task** actions navigate away from those pages. Notification mode requests macOS notification permission and groups tasks that finish in the same refresh into one notification; clicking it opens the inbox. If permission is denied, the inbox explains how to enable it and still collects completions. The inbox collects completions observed while the utility is running; the initial snapshot is a baseline, so old task history is not imported or announced. Use
...
- Threads are ordered by Codex's indexed recency metadata, avoiding historical rollout scans. All threads active during the current Codex process remain in the catalog and have their latest lifecycle envelope inspected, even beyond the usual 500-thread history limit. The renderer initially mounts 60 matching threads and exposes **Load more tasks** in 60-thread pages to keep the DOM responsive.
- Threads are grouped into collapsible projects. **Today** is the default dashboard view, alongside **Running**, **Unread**, and **Changed projects** filters.
...
- The menu bar provides dashboard, restart, disable, compatibility, diagnostics, the completion inbox, completion behaviour, and launch-at-login actions.

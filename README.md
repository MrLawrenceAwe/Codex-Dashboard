...
- The active account—including an unsaved current account—shows its five-hour and weekly Codex usage remaining, with a countdown to each reset. A persistent, properly initialized local app-server session uses Codex's canonical active home, is restarted on account changes, and refreshes every minute while active or every five minutes in the background. Polling never opens inactive credentials in Keychain, and failed refreshes visibly mark prior usage as stale.

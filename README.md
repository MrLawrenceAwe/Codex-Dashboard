...
- Changed project groups expose **Mute changes** and **Commit or push**. Mute changes hides that project's change notifications until restored; Commit or push opens the most recent idle thread only when Codex's native Git command is available.
...
- Compatibility checks run automatically after the installed Codex version changes. Warnings and incompatibilities replace the normal menu-bar icon, appear in the menu and Diagnostics, and request a macOS notification; blocking drift prevents remounting until it is reviewed.
...
- Injection startup and shared page navigation live in `Core`; Task Dashboard and To-dos own their page controllers, with To-do markup and rendering in `todo-list-view.js`.

...
- Threads are grouped into collapsible projects. **All** is the default dashboard view, and search covers idle clean threads as well as running, unread, and changed work.
...
- Grouped projects show a quiet marker when their Git working tree has uncommitted changes, and the **Changed projects** filter isolates those projects. Running dirty projects remain visible there, while Commit or push waits for an idle project thread.
...
- The active account—including an unsaved current account—shows its five-hour and weekly Codex usage remaining, countdowns to each timed reset, and the number of available banked resets with the nearest known expiry. A persistent, properly initialized local app-server session uses Codex's canonical active home, is restarted on account changes, and refreshes every minute while active or every five minutes in the background. Polling never opens inactive credentials in Keychain, and failed refreshes visibly mark prior usage as stale.

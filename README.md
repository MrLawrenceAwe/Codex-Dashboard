...
- Threads are ordered by Codex's indexed recency metadata, avoiding historical rollout scans. All threads active during the current Codex process remain in the catalog and have their latest lifecycle envelope inspected, even beyond the usual 500-thread history limit. The renderer initially mounts 60 matching threads and exposes **Load more threads** in 60-thread pages to keep the DOM responsive.
